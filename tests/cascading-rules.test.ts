...
describe('Cascading Rules (XComponent-style)', () => {
  ...
  describe('Cascading with Guards', () => {
    const component: Component = {
      name: 'GuardedCascadeComponent',
      version: '1.0.0',
      stateMachines: [
        {
          name: 'Order',
          ...
          states: [
            { name: 'Pending', type: StateType.ENTRY },
            {
              ...
              cascadingRules: [
                {
                  targetMachine: 'Notification',
                  targetState: 'Idle',
                  event: 'SEND',
                  matchingRules: [
                    {
                      eventProperty: 'userId',
                      instanceProperty: 'userId',
                    },
                  ],
                  payload: {
                    userId: '{{UserId}}',
          ...
          transitions: [
            {
              ...
              guards: [
                {
                  customFunction: 'context.Amount < 1000',
                },
              ],
            },
          ],
        },
        {
          name: 'Notification',
          initialState: 'Idle',
          publicMemberType: 'Notification',
          states: [
            { name: 'Idle', type: StateType.ENTRY },
            { name: 'Sent', type: StateType.REGULAR },
          ],
          transitions: [
            {
              from: 'Idle',
    ...
    it('should only cascade when guard passes', async () => {
      const runtime = new FSMRuntime(component);

      const notificationId = runtime.createInstance('Notification', { userId: 'U1' });

      // Order with amount < 1000 (guard passes)
      ...
      // Should cascade
      expect(runtime.getInstance(notificationId)?.currentState).toBe('Sent');
    });

    it('should not cascade when guard fails', async () => {
      const runtime = new FSMRuntime(component);

      const notificationId = runtime.createInstance('Notification', { userId: 'U2' });

      // Order with amount >= 1000 (guard fails)
      ...
      // Should NOT cascade (order didn't transition)
      expect(runtime.getInstance(order2)?.currentState).toBe('Pending');
      expect(runtime.getInstance(notificationId)?.currentState).toBe('Idle');
    });
  });
